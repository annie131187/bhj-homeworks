...
prodItems.forEach(item => {
    item.addEventListener('click', evt => {
        ...
        if (evt.target.classList.contains('product__add')) {
            let cartItem = item.querySelector('.cart__product');
            if (!cartItem) {
            ...
            `);
            } else if (cartItem && cartItem.dataset.id == item.dataset.id) {
                let cartQuantity = cartItem.querySelector('.cart__product-count');
